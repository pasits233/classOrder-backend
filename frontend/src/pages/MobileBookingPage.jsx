...
export default function MobileBookingPage() {
  ...
  const handleCoachChange = (coachId) => {
    ...
    if (coachId && selectedDate) {
      fetchUnavailableSlots(coachId, selectedDate.format('YYYY-MM-DD'));
    } else {
      setUnavailableSlots([]);
    }
    setSelectedSlots([]);
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="date" label="日期" rules={[{ required: true, message: '请选择日期' }]}> 
            <DatePicker style={{ width: '100%' }} value={selectedDate} onChange={handleDateChange} />
          </Form.Item>
          <Form.Item label="时间段" required>
            {selectedDate ? (
              ...
              <div style={{ color: 'red' }}>请先选择日期</div>
            )}
